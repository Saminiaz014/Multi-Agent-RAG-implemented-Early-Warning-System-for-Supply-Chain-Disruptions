# Development Log — Phase-by-Phase Build History

This is the original phase-structured build log that formed the body of
`README.md` up to 2026-09-05. It is preserved verbatim because it records
*how* the system was built, in the order it was built, including the
reasoning behind decisions that were later revised.

The README itself is now organised around the thesis chapter structure
(Methodology, Evaluation) rather than build phases. Use this file when you
need the construction history; use the README when you need the current
state of the system.

**Caveat.** Entries describe the state at the time each phase was completed.
Several are superseded by later work — most importantly, the Ambee disaster
path (Phase 7) is dead and was replaced by GDACS + USGS in Phase 13, and the
test counts quoted in early phases are point-in-time. Where a phase entry and
