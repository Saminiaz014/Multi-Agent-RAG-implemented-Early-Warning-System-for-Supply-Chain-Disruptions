...
def aggregate_ablations() -> None:
    """Load all ablation result JSONs, compute mean over scenarios, print + save a summary."""
    json_files = sorted(_RESULTS_DIR.glob("*.json"))
    if not json_files:
        print(f"No ablation results found in {_RESULTS_DIR}. Run scripts/run_ablations.py first.")
        return
    ...
    print("\n" + "=" * 90)
    print("ABLATION SUMMARY (mean over Hormuz 4 scenarios, seed=42)")
    print("=" * 90)
    ...
    output_file = _RESULTS_DIR / "ablation_summary.csv"
