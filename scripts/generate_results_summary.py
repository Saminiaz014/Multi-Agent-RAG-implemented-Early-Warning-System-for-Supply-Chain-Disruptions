...
``scripts/aggregate_all_results.py`` (run that first, for the same
``--region``). Nothing here is invented: metrics that were never computed
(D1/D2 VUS, D11-D15 event/range/PA-F1 variants) are listed as such with a
reason, and there is no "full system" / pre-declared-target section, since
no such run or spec exists in this repo — see the Limitations section this
script writes.

Run from project root, after ``scripts/aggregate_all_results.py``::

    python scripts/generate_results_summary.py [--region REGION]

``--region`` defaults to ``hormuz`` (matching every R4-R8 script). It reads
the region-suffixed CSVs (``results_all_runs_{region}.csv``, etc.) that
``aggregate_all_results.py`` writes, so a second region's numbers never
silently blend into this region's narrative — this script was found to be
a third unfiltered aggregation-layer entry point (2026-08-16, downstream of
the ``aggregate_all_results.py``/``aggregate_ablation_results.py`` fix), see
...
def load_summary_data(region: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Load ``region``'s aggregated CSVs, as written by ``aggregate_all_results.py``.

    Args:
        region: Canonical region key (already resolved via
            :func:`~src.benchmark.regions.resolve_region_key`).
    """
    df_all = pd.read_csv(_RESULTS_ROOT / f"results_all_runs_{region}.csv")
    df_by_baseline = pd.read_csv(_RESULTS_ROOT / f"results_by_baseline_{region}.csv", index_col=0)
    df_by_scenario = pd.read_csv(_RESULTS_ROOT / f"results_by_scenario_{region}.csv", index_col=0)
    df_ablations = pd.read_csv(_RESULTS_ROOT / f"ablation_findings_{region}.csv", index_col=0)
...
def generate_summary_md(
    ...
    return f"""# Results Summary: EVAL01 Baseline & Ablation Benchmark (region={region})

*Generated by `scripts/generate_results_summary.py --region {region}` from
`results/results_all_runs_{region}.csv`. Every number below is read from
that file, filtered to this region only — nothing here is invented.*
...
{region} scenarios x 5 seeds) and **aggregation strategy value** (ablations
...
Full table with std: `results/results_by_baseline_{region}.csv`.
...
Full per-scenario detail: `results/ablation_findings_{region}.csv` and
`results/baselines/ablations/{region}_*.json`.
...
def main(region: str = "hormuz") -> None:
    """Generate ``region``'s results summary markdown from its aggregated CSVs.

    Args:
        region: Canonical region key, alias, or display name (see
            :func:`src.benchmark.regions.resolve_region_key`). Defaults to
            ``"hormuz"``, matching every R4-R8 script.
    """
    region = resolve_region_key(region)

    logger.info("Loading aggregated results for region=%s...", region)
    df_all, df_by_baseline, df_by_scenario, df_ablations = load_summary_data(region)

    logger.info("Computing findings from measured data...")
    findings = compute_key_findings(df_all)

    logger.info("Generating markdown...")
    summary_md = generate_summary_md(findings, df_by_baseline, df_by_scenario, df_ablations, region)

    output_file = _RESULTS_ROOT / f"benchmark_summary_{region}.md"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate a thesis-ready results summary markdown for a region.",
    )
    parser.add_argument(
        "--region",
        default="hormuz",
        help="Region key, alias, or display name to summarize (default: hormuz). "
        "Reads that region's aggregated CSVs only, so summaries never silently "
        "mix regions.",
    )
